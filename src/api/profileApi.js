...
export const profileApi = {
  ...
  getMyProfile: async (userId, token) => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/sns/users/myprofile?userId=${userId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  uploadProfileImage: async (userId, imageUri, token) => {
    try {
      ...
      const response = await axios.post(`${API_URL}/api/sns/profile/upload-profile-image`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  // 프로필 이미지 수정
  updateProfileImage: async (userId, imageUri, token) => {
    try {
      console.log('[ProfileApi] Updating profile image. Details:', {
        userId,
        imageUri,
        hasToken: !!token,
      })
      ...
      const response = await axios.put(`${API_URL}/api/sns/profile/update-profile-image`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'multipart/form-data',
        },
      ...
      console.error('[ProfileApi] Update profile image error:', error)
      throw error.response?.data || error.message
    }
  },

  // 프로필 이미지 삭제
  deleteProfileImage: async (userId, token) => {
    try {
      console.log('[ProfileApi] Deleting profile image for user:', userId)

      const response = await axios.delete(`${API_URL}/api/sns/profile/delete-profile-image`, {
        params: { userId },
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      console.log('[ProfileApi] Delete response:', response.data)
      return response.data
    } catch (error) {
      console.error('[ProfileApi] Delete profile image error:', error)
      throw error.response?.data || error.message
    }
  },
}
