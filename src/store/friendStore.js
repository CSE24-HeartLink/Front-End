...
const useFriendStore = create((set, get) => ({
  ...
  deleteFriend: async (friendId) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // 상대방의 친구 목록에서도 즉시 제거
        // API에서 제공하는 양방향 관계 ID를 사용
        if (result.data?.relationId) {
          await friendApi.updateFriendListAfterDelete(targetUserId, result.data.relationId)
        }
  ...
  //친구 그룹 수정
  updateFriendGroup: async (friendId, groupId) => {
    set((state) => ({
      friends: state.friends.map((friend) => (friend.friendId._id === friendId ? { ...friend, group: groupId } : friend)),
    }))
  },

  // 친구 신청 수락 시 친구 목록 즉시 업데이트를 위한 함수
  // friendData: { _id, friendId: { _id, nickname, profileImage }, group }
  updateFriendsList: (friendData) => {
    set((state) => ({
