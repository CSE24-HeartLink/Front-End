...
const useFeedStore = create((set, get) => ({
  ...
  updateFeed: async (feedId, updateData) => {
    try {
      ...
      if (result.feed) {
        ...
        set((state) => ({
          feeds: state.feeds.map((feed) => (feed.feedId === feedId ? result.feed : feed)),
          filteredFeeds: state.filteredFeeds.map((feed) => (feed.feedId === feedId ? result.feed : feed)),
          isLoading: false,
          error: null,
        }))
  ...
  deleteFeed: async (feedId) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log('[FeedStore] Updating local state after successful deletion')
        set((state) => ({
          feeds: state.feeds.filter((feed) => feed.feedId !== feedId),
          filteredFeeds: state.filteredFeeds.filter((feed) => feed.feedId !== feedId),
          error: null,
          isLoading: false,
        }))
  ...
  //토글 리액션
  toggleReaction: async (feedId, reactionType) => {
    try {
      const userId = useAuthStore.getState().getUserId()
      if (!userId) throw new Error('인증 정보가 없습니다.')
